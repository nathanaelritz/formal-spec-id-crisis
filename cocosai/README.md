...
|------------------|-----------------|------------------|
| **Vulnerable**   | `dhe/cve/`      | `mlkem/cve/`     |
| **Mitigated**    | `dhe/fix/`      | `mlkem/fix/`     |
...
## Verification Outcomes: DHE vs. ML-KEM Parity
...
3. **Primitive-Level Deltas:** The only deviations reflect expected, structurally necessary differences between the abstractions. The idealized DHE models include escape conditions for weak subgroup attacks (`SentBadElement`, `ServerChoosesKEX(WeakDH,...)`), which have no KEM equivalent and are correctly absent from the KEM models. The session binding value is `kem_ss` in place of `gxy`. No security-relevant result flips between columns.
